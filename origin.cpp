...
double value1(TankGame::TankField &field){  //只有这里需要重写
    ...
    const int tank_alive = 100;
    const int base_alive = 100000;
    ...
    if(field.baseAlive[me])res += base_alive;
    if(field.baseAlive[enemy])res -= base_alive;
    int x1=field.tankX[me][0],x2=field.tankX[me][1];
    if(x1!=-1){
        res -= abs(field.tankX[me][0]-baseX[1-me]) + 2*abs(field.tankY[me][0]-baseY[1-me]);
    }
    if(x2!=-1){
        res -= abs(field.tankX[me][1]-baseX[1-me]) + 2*abs(field.tankY[me][1]-baseY[1-me]);
    }
    return res;
}
...
public:
    ...
    pair<TwoActions, double> alpha_beta_search(TankGame::TankField &field, double (*value)(TankGame::TankField &f), double alpha, double beta, int depth, bool isenemy, TwoActions pre){
        using namespace TankGame;
        GameResult result = field.GetGameResult();
        if(result==Draw)return make_pair(TwoActions(), 0);
        if(depth==0 || result != NotFinished){
            double val = value(field);
            return make_pair(TwoActions(), val);
        }
        vector<TwoActions> myValidActions;
        vector<TankGame::Action> my[2];
        int me=field.mySide;
        if(isenemy)me = 1 - me;
        ...
        for(auto action: myValidActions){
            if(isenemy){
                ...
                field.DoAction();
                double u = alpha_beta_search(field, value, alpha, beta, depth-1, false, TwoActions()).second;
                if(beta > u){
                    beta = u;
                    response = action;
                }
                ...
                double u = alpha_beta_search(field, value, alpha, beta, depth, true, action).second;
                if(alpha < u){
                    alpha = u;
                    response = action;
                }
        ...
        if(isenemy)return make_pair(response, beta);
        else return make_pair(response, alpha);
    }
    TwoActions run(TankGame::TankField &field, double (*value)(TankGame::TankField &f)){
        return alpha_beta_search(field, value, -INF, INF, search_depth_limit, false, TwoActions()).first;
    }
};
